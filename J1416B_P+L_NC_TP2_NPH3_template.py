...
# Give the run name
runname = "J1416B_P+L_NC_TP2_NPH3"

obspec = np.asfortranarray(np.loadtxt("J1416B_PRISM+MIRI.txt", dtype='d', unpack='true'))
...
outdir = "/Users/917543996/Saves/J1416B/NIRSpecMIRI/NC_TP2_NPH3/run_1/"
...
#    p0[:, 6] = (0.5 * np.random.randn(nwalkers).reshape(nwalkers)) - 8.0  # PH3
    p0[:, 6] = (0.5 * np.random.randn(nwalkers).reshape(nwalkers)) - 6.0  # Na+K
    ...
    p0[:, 9] = np.random.randn(nwalkers).reshape(nwalkers) * 0.001  # dlambda
    p0[:, 10] = np.log10((np.random.rand(nwalkers).reshape(nwalkers) * (max(obspec[2, :] ** 2) * (0.1 - 0.01))) + (
...
#   p0[:,ndim-13 + i] = (BTprof[i] - 200.) + (150. * np.random.randn(nwalkers).reshape(nwalkers))
    ...
    p0[:,11] = 0.39 + 0.1*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,12] = 0.14 +0.05*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,13] = -1.2 + 0.2*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,14] = 2.25+ 0.2*np.random.randn(nwalkers).reshape(nwalkers)
    p0[:,15] = 4200. + (500.*  np.random.randn(nwalkers).reshape(nwalkers))
    for i in range (0,nwalkers):
        while True:
            Tcheck = TPmod.set_prof(proftype, coarsePress, press, p0[i, ndim-5:])
            if min(Tcheck) > 1.0:
...
# need to get wavenum and  temps size for linelist array
