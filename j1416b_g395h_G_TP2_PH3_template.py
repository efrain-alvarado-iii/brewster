...
# Give the run name
runname = "J1416B_G395H_G_TP2_PH3"

# get the observed spectrum
obspec = np.asfortranarray(np.loadtxt("convert_units_J1416B.txt",dtype='d',unpack=True))
...
# give the full path
xpath = "/Users/917543996/Linelists/R30Kopacities/"
...
outdir = "/Users/917543996/Saves/J1416B/G395H/G_TP2_PH3/run_2/"
...
if (fresh == 0):
    ...
    p0[:,7] = 0.1*np.random.randn(nwalkers).reshape(nwalkers) + 5  # gravity
...
# ------ If you have a cloud, you will always need cloud parameters. ------
  ...
  #  p0[:,19] = 0.1*np.random.rand(nwalkers).reshape(nwalkers) # Hansen b for "real" cloud or Power law for 89/99 cloud
    ...
    # e.g grey cloud or power law cloud, or no cloud, or deck cloud
    # this example is a "real" cloud with Hansen a and b parameters
    p0[:,12] = 0.5+ 0.2* np.random.rand(nwalkers).reshape(nwalkers) # cloud top pressure
    p0[:,13] = np.random.rand(nwalkers).reshape(nwalkers) # cloud thickness in pressure
    p0[:,14] = np.random.randn(nwalkers).reshape(nwalkers) # Hansen a if "real" cloud or single scattering albedo between 0 and 1 (np.rand=uniform distribution) for 89/99 cloud
    #p0[:,15] = np.abs(0.1+ 0.01*np.random.randn(nwalkers).reshape(nwalkers)) # Hansen b for "real" cloud or Power law for 89/99 cloud
...
#   p0[:,ndim-13 + i] = (BTprof[i] - 200.) + (150. * np.random.randn(nwalkers).reshape(nwalkers))   
    ...
    p0[:,19] = 3000. + (500.*  np.random.randn(nwalkers).reshape(nwalkers))
    for i in range (0,nwalkers):
        while True:
